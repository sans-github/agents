...
- **Smoke test handoff:** after deployment, hand off the live server URL to QA and remain available to receive failure reports; when QA reports a failing smoke test case with specific error details, investigate and remediate promptly -- do not declare deployment complete until QA confirms all smoke checks pass
